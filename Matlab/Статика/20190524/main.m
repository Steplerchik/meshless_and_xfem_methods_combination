...
lc = 0.3*a;
teta = 15;  % По Цельсию
Kin1 = 0.9904*sqrt(pi*lc/2);
Kin2 = 0.2527*sqrt(pi*lc/2);
...
% Yc = [yc1 yc2 99.7974];
% Xc = [xc1 xc2 31.8682];
Yc = [yc1 yc1-0.5*step*sin(teta/180*pi) yc2+0.5*step*sin(teta/180*pi) yc2];
Xc = [xc1 xc1-0.5*step*cos(teta/180*pi) xc2+0.5*step*cos(teta/180*pi) xc2];
% Yc = [yc1 yc2 91.5762];
% Xc = [xc1 xc2 29.9577];
% Yc = [yc1 yc2 91.1687];
% Xc = [xc1 xc2 29.9140];
...
% lx = length(Xc);
% X = [X, Xc(1)+0.5*step*cos(atan((Yc(1)-Yc(2))/(Xc(1)-Xc(2)))+30/180*pi) Xc(1)+0.5*step*cos(atan((Yc(1)-Yc(2))/(Xc(1)-Xc(2)))-30/180*pi)];
% Y = [Y, Yc(1)+0.5*step*sin(atan((Yc(1)-Yc(2))/(Xc(1)-Xc(2)))+30/180*pi) Yc(1)+0.5*step*sin(atan((Yc(1)-Yc(2))/(Xc(1)-Xc(2)))-30/180*pi)];
% X = [X, Xc(lx)-0.5*step*cos(atan((Yc(lx-1)-Yc(lx))/(Xc(lx-1)-Xc(lx)))+30/180*pi) Xc(lx)-0.5*step*cos(atan((Yc(lx-1)-Yc(lx))/(Xc(lx-1)-Xc(lx)))-30/180*pi)];
% Y = [Y, Yc(lx)-0.5*step*sin(atan((Yc(lx-1)-Yc(lx))/(Xc(lx-1)-Xc(lx)))+30/180*pi) Yc(lx)-0.5*step*sin(atan((Yc(lx-1)-Yc(lx))/(Xc(lx-1)-Xc(lx)))-30/180*pi)];
...
Kin_tip1_percent = [abs(Kin1_tip1_(1,1)-Kin1)/Kin1 abs(abs(Kin1_tip1_(1,2))-Kin2)/Kin2]
